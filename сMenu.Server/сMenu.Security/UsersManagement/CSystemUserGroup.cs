    ...
    public class CSystemUserGroup : CMetaobject
    {
        ...
        public static List<CSystemUserGroup> sGetAllGroups(IDatabaseProvider Provider)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
            {
                ...
                Group.Key = T.Rows[i][0].PostProcessValue<decimal>();
                Group.ID = Guid.Parse(T.Rows[i][1].PostProcessValue<string>());
                Group.Parent = T.Rows[i][2].PostProcessValue<decimal>();
                Group.Class = (EnMetaobjectClass)T.Rows[i][3].PostProcessValue<int>();
                Group.System = (T.Rows[i][4].PostProcessValue<int>() == 1);
                Group.ModificatonDate = T.Rows[0][5].PostProcessValue<DateTime>();
                Group.Status = (EnMetaobjectStatus)T.Rows[0][6].PostProcessValue<int>();
