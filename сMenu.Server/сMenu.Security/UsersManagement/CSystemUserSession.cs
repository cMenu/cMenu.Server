...
namespace cMenu.Security.UsersManagement
{
    ...
    public class CSystemUserSession
    {
        ...
        public int SessionGetByKey(decimal Key, IDatabaseProvider Provider)
        {
            ...
            this._userKey = T.Rows[0][0].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
            this._deadLine = T.Rows[0][2].PostProcessValue<DateTime>(DateTime.Now);
            this._key = T.Rows[0][3].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
            this._id = Guid.Parse(T.Rows[0][4].PostProcessValue<string>(""));
            this._status = T.Rows[0][5].PostProcessValue<EnSessionStatus>(EnSessionStatus.EEnabled);
            this._type = T.Rows[0][7].PostProcessValue<EnSessionType>(EnSessionType.EWeb);

            byte[] Data = T.Rows[0][6].PostProcessValue<byte[]>(new byte[0]);
        ...
        public static List<CSystemUserSession> sGetSessionsByUser(decimal UserKey, IDatabaseProvider Provider)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
            {
                ...
                Session.UserKey = T.Rows[i][0].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
                Session.DeadLine = T.Rows[i][1].PostProcessValue<DateTime>(DateTime.Now);
                Session.Key = T.Rows[i][3].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
                Session.ID = Guid.Parse(T.Rows[i][4].PostProcessValue<string>(""));
                Session.Status = T.Rows[i][5].PostProcessValue<EnSessionStatus>(EnSessionStatus.EEnabled);
                Session.Type = T.Rows[i][7].PostProcessValue<EnSessionType>(EnSessionType.EWeb);
