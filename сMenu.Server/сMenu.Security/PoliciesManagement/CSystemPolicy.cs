    ...
    public class CSystemPolicy : CSecuredMetaobject
    {
        ...
        public List<CSystemPolicy> sGetAllPolicies(IDatabaseProvider Provider)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
            {
                ...
                Policy.Key = T.Rows[i][0].PostProcessValue<decimal>();
                Policy.ID = Guid.Parse(T.Rows[i][1].PostProcessValue<string>());
                Policy.Parent = T.Rows[i][2].PostProcessValue<decimal>();
                Policy.Class = T.Rows[i][3].PostProcessValue<EnMetaobjectClass>();
                Policy.System = ((T.Rows[i][4]).PostProcessValue<int>() == 1);
                Policy.ModificatonDate = T.Rows[0][5].PostProcessValue<DateTime>();
                Policy.Status = T.Rows[0][6].PostProcessValue<EnMetaobjectStatus>();
