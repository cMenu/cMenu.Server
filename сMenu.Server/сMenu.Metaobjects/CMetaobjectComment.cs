    ...
    public class CMetaobjectComment : CBaseEntity
    {
        ...
        public List<CMetaobjectComment> GetChildren(IDatabaseProvider Provider, bool Recursive = false)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
            {
                ...
                Comment.Key = T.Rows[0][0].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
                Comment.ID = Guid.Parse(T.Rows[0][1].PostProcessValue<string>(""));
                Comment.ObjectKey = T.Rows[0][2].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
                Comment.Date = T.Rows[0][3].PostProcessValue<DateTime>(DateTime.Now);
                Comment.Author = T.Rows[0][4].PostProcessValue<string>("");
                Comment.AuthorEmail = T.Rows[0][5].PostProcessValue<string>("");
                Comment.Comment = T.Rows[0][6].PostProcessValue<string>("");
                Comment.Parent = T.Rows[0][7].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
                if (Recursive)
        ...
        public int CommentGetByKey(IDatabaseProvider Provider)
        {
            ...
            this._key = T.Rows[0][0].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
            this._id = Guid.Parse(T.Rows[0][1].PostProcessValue<string>(""));
            this._objectKey = T.Rows[0][2].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
            this._date = T.Rows[0][3].PostProcessValue<DateTime>(DateTime.Now);
            this._author = T.Rows[0][4].PostProcessValue<string>("");
            this._authorEmail = T.Rows[0][5].PostProcessValue<string>("");
            this._comment = T.Rows[0][6].PostProcessValue<string>("");
            this._parent = T.Rows[0][7].PostProcessValue<decimal>(CDBConst.CONST_OBJECT_EMPTY_KEY);
        ...
        public static int sDeleteCommentsByParent(decimal Key, IDatabaseProvider Provider)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
                CMetaobjectComment.sDeleteCommentsByParent(T.Rows.PostProcessValue<decimal>(), Provider);
