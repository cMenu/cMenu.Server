...
namespace cMenu.Rds
{
    ...
    public class CRdsAttributeValue
    {
        ...
        public int ValueGet(int LanguageCode, IDatabaseProvider Provider)
        {
            ...
            byte[] Data = T.Rows[0][3].PostProcessDatabaseValue<byte[]>();
        ...
        public int ValueGetAll(IDatabaseProvider Provider)
        {
            ...
            {
                byte[] Data = T.Rows[i][3].PostProcessDatabaseValue<byte[]>();
                var R = Data.ToDataStream().DeserializeBinaryStream<object>();
                this._values.Add(T.Rows[i][2].PostProcessDatabaseValue<int>(), R);
            }
        ...
        public bool ValueExists(int LanguageCode, IDatabaseProvider Provider)
        {
            ...
            return (T.Rows[0][0].PostProcessDatabaseValue<int>() > 0);
        }
        #endregion
        ...
        public static List<CRdsAttributeValue> sGetAttributeValuesByElement(decimal ElementVersion, IDatabaseProvider Provider)
        {
            ...
            for (int i = 0; i < T.Rows.Count; i++)
            {
                CurrentKey = T.Rows[i][1].PostProcessDatabaseValue<decimal>(-1);
                if (PreviousKey != CurrentKey)
                {
                    ...
                    Value.ElementVersion = T.Rows[i][0].PostProcessDatabaseValue<decimal>(-1);
                    Value.AttributeKey = T.Rows[i][1].PostProcessDatabaseValue<decimal>(-1);
